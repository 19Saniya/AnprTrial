...
cursor = conn.cursor()
cursor.execute('''
    CREATE TABLE IF NOT EXISTS number_plate_trial3b (
...
def process_frame():
    ...
    for (x, y, w, h) in plates:
        ...
        if plate_text:
            ...
            plate_img_bytes = io.BytesIO()
            plate_img_pil = Image.fromarray(plate_img)
            plate_img_pil.save(plate_img_bytes, format='PNG')
            cursor.execute('INSERT INTO number_plate_trial3b (plate_text, capture_time, plate_image) VALUES (?, ?, ?)',
                           (plate_text, capture_time, plate_img_bytes.getvalue()))
            conn.commit()
...
def update_plate_list():
    cursor.execute('SELECT * FROM number_plate_trial3b')
    ...
    for plate in plates:
        ...
        plate_img_bytes = io.BytesIO(plate[2])
