...
with sqlite3.connect('number_plates.db') as conn:
    cursor = conn.cursor()
    # cursor.execute('''
    #     CREATE TABLE IF NOT EXISTS number_plate_trial1e (
...
with sqlite3.connect('number_plates.db') as conn:
    cursor = conn.cursor()
    cursor.execute('SELECT plate_text, plate_image FROM number_plate_trial1b')
